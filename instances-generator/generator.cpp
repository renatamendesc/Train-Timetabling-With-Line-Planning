...
void generate_arcs (int max_time, int nb_points, vector <int> &depots, vector<vector<int>> &adj_matrix)
{
    ...
    // add a margin to each arc and assign values to the matrix
...
void generate_routes (vector <vector <int>> &routes, vector <int> &depots, int nb_points)
{
    ...
    // cyclic routes starting from the extremities
    ...
    // non-cyclic routes connecting middle depots with extremities
    ...
    // non-cyclic routes connecting adjacent depots
...
{
    cout << "number of trains: " << nb_trains << endl;
    ...
    cout << "number of depots: " << depots.size() << endl << "\t";
    for (auto d : depots)
    {
        cout << d << " ";
    }
    ...
    cout << "number of crossings: " << crossings.size() << endl << "\t";
    for (auto c : crossings)
    {
        cout << c << " ";
    }
    ...
    cout << "initial depot: " << initial_depot << endl;
    cout << "maximum operation time: " << max_time << " (" << max_time / 3600 << " hours)" << endl << endl;

    for (int i = 0; i < time_intervals.size(); i++)
    {
        cout << "interval " << i << ": (" << get <0> (time_intervals[i]) << ", " << get <1> (time_intervals[i]) << ")" << endl;
    }
    cout << endl;
...
int main(int argc, char *argv[])
{
    ...
    const filesystem::path experiments_set_dir("new_instances/" + set_name);
    if (!filesystem::exists(experiments_set_dir))
        filesystem::create_directories(experiments_set_dir);
    ...
    for (int p = points_range.first; p <= points_range.second; p++)
    {
        for (int t = trains_range.first; t <= trains_range.second; t++)
        {
            ...
            for (const auto& entry : filesystem::directory_iterator("new_instances/" + set_name)) {
            ...
            while (not_feasible)
            {
                ...
                if (solved)
                {
                    ...
                    string dir_path = "new_instances/" + set_name;
                    ...
                    try
                    {
                        ...
                        cout << "Instance created successfully! - " << p << ", " << t << endl;
                    }
